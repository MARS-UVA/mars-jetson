...
def max_drum_spin(front_arm: bool, back_arm: bool, msg: MotorChanges, forward: bool) -> None:
    if front_arm:
        if forward:
            msg.changes.append(SetMotor(index=AddMotor.SPIN_FRONT_DRUM, velocity=254))
        else:
            msg.changes.append(SetMotor(index=AddMotor.SPIN_FRONT_DRUM, velocity=0))
    if back_arm:
        if forward:
            msg.changes.append(SetMotor(index=AddMotor.SPIN_BACK_DRUM, velocity=254))
        else:
            msg.changes.append(SetMotor(index=AddMotor.SPIN_BACK_DRUM, velocity=0))
