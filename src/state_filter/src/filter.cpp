...
namespace filter {
    ...
    void unscented_transform(const Eigen::Matrix<T, vars, K>& sigma_points /* X */, const Eigen::Matrix<T, K, 1>& mean_weights /* W(m) */, const Eigen::Matrix<T, K, 1>& covariance_weights /* W(c) */, const Eigen::Matrix<T, vars, vars>& additional_covariance_sqrt /* Q or R */, Eigen::Matrix<T, vars, 1>& mean /* ? -> x_hat */, Cholesky<Eigen::Matrix<T, vars, vars>>& covariance_cholesky /* ? -> S */, Eigen::Matrix<T, vars, K>& distance /* ? -> X_i - x_hat */) {
        ...
        if (covariance_cholesky.info() == Eigen::NumericalIssue) {
            std::cerr << "WARNING: State covariance estimate has diverged.\n";
        }
    }
    ...
    void RobotModelFilter::predict(const double& dt, const StateCovariance& process_model_covariance_sqrt) {
        ...
        unscented_transform<double, STATE_VARS, STATE_THETA>(transformed_sigma_points, _mean_weights, _covariance_weights, process_model_covariance_sqrt, _state_estimate, _state_covariance_estimate_cholesky, _prediction_residual);
    }
    ...
    void update_from_observation(const Eigen::Matrix<double, vars, 1>& observation /* y_hat_curr */, const Eigen::Matrix<double, vars, vars>& covariance_sqrt /* R_curr */, const std::function<Eigen::Matrix<double, vars, 1>(State)>& observation_model /* h_curr */, const SigmaPoints& sigma_points /* X_prev */, const Weights& mean_weights /* W(m) */, const Weights& covariance_weights /* W(c) */, State& state_estimate /* x_hat_prev -> x_hat_curr */, Cholesky<StateCovariance>& state_estimate_covariance_cholesky /* P_prev -> P_curr */) {
        ...
        unscented_transform<double, vars, theta_index>(observation_sigma_points, mean_weights, covariance_weights, covariance_sqrt, observation_prediction, observation_prediction_covariance_cholesky, observation_distance);
    ...
    void RobotModelFilter::update_from_apriltag(const AprilTagObservation &observation, const AprilTagObservationCovariance& covariance_sqrt) {
        ...
        update_from_observation<APRILTAG_VARS, APRILTAG_THETA>(observation, covariance_sqrt, _apriltag_observation_model, _sigma_points, _mean_weights, _covariance_weights, _state_estimate, _state_covariance_estimate_cholesky);
    }

    void RobotModelFilter::update_from_gyro(const GyroObservation &observation, const GyroObservationCovariance& covariance_sqrt) {
        compute_sigma_points(_state_estimate, _state_covariance_estimate_cholesky, _eta, _sigma_points);

        update_from_observation<GYRO_VARS, -1>(observation, covariance_sqrt, _gyro_observation_model, _sigma_points, _mean_weights, _covariance_weights, _state_estimate, _state_covariance_estimate_cholesky);
    }
